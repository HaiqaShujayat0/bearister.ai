...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    console.log("Submitting password reset with token:", token ? token.substring(0, 20) + "..." : "NO TOKEN")
    ...
    try {
      console.log("Calling API resetPassword with token length:", token.length)
          ...
          <div className="bg-[#202939] rounded-2xl pt-10 pr-8 pb-8 pl-8 border border-[#364152] shadow-lg flex flex-col gap-6" style={{ transform: 'scale(var(--card-scale))', transformOrigin: 'top center' }}>
            ...
            {!debugMode && !token && (
              <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-md">
                <p className="text-sm text-yellow-400">Reset token missing. Please open the password reset link from your email again.</p>
              </div>
            )}
            ...
            {!success ? (
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                {debugMode && (
                  <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-md">
                    <p className="text-sm text-yellow-400 mb-2">Debug Mode Active</p>
                    <button
                      type="button"
                      onClick={() => {
              ...
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-[#cdd5df] mb-8 text-center">Your Password has been changed successfully</p>
                <button 
